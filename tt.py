...
vector_configs = ['', '-Dllama2.VectorFloat16=true', '-Dllama2.VectorFloat8=true', '-Dllama2.VectorFloat4=true']

# Range of inputs
input_range = [i for i in range(50, 501) if 32000 % i == 0]  # Numbers that divide 32000 with remainder 0
...
for dataset in datasets:
    ...
    for vector_config in vector_configs:
        ...
        for dim in input_range:
            ...
            base_command = 'tornado --jvm=" {} -Dworkgroup.size={} " -cp target/tornadovm-llama-gpu-1.0-SNAPSHOT.jar io.github.mikepapadim.Llama2 {}.bin'.format(vector_config, dim, dataset)
            ...
            for _ in range(num_runs):
                ...
                #time.sleep(1)  # Add a delay of 1 secondI
                ...
                if match:
                    ...
                    print(f"Warning: No match found in output for dim={dim}. Output:\n{result.stdout}")

            if dim_values:
                # Calculate and print the average and geometric mean for the current input
                avg_value = sum(dim_values) / len(dim_values)
                geo_mean_value = (np.prod(dim_values))**(1/len(dim_values))

                print(f"Dataset: {dataset}, Vector Config: {vector_config}, Input: {dim}, Average tok/s: {avg_value}, Geometric mean tok/s: {geo_mean_value}")

                # Store the values for further analysis or plotting
                toks_per_sec_values.append(dim_values)
            else:
                print(f"Dataset: {dataset}, Vector Config: {vector_config}, Input: {dim}, No valid tok/s values found.")
        ...
        with open(output_file, mode='w', newline='') as file:
            ...
            writer.writerow(['Input', 'Average tok/s', 'Geometric mean tok/s'])

            for i, dim in enumerate(input_range):
                if i < len(toks_per_sec_values) and toks_per_sec_values[i]:
                    avg_value = sum(toks_per_sec_values[i]) / len(toks_per_sec_values[i])
                    geo_mean_value = (np.prod(toks_per_sec_values[i]))**(1/len(toks_per_sec_values[i]))
                    writer.writerow([dim, avg_value, geo_mean_value])
                else:
